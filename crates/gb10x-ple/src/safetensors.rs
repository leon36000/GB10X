...
pub(crate) struct ParsedHeader {
    ...
    pub(crate) tensors: Vec<ParsedTensor>,
}
...
fn parse_header(bytes: &[u8]) -> Result<ParsedHeader, PlePackIoError> {
    ...
    let data_section_start = data_section_start(header_len)?;
    ...
    parse_header_json(
        ...
        u64::try_from(bytes.len())
            .map_err(|_| PlePackIoError::Format("safetensors file length exceeds u64"))?,
    )
}

/// Read and validate only a safetensors prefix and JSON header from an open file.
///
/// The reader never maps or materializes the payload. `max_header_bytes` bounds the JSON header
/// alone; the returned `header_bytes_read` includes the eight-byte length prefix.
pub(crate) fn read_safetensors_header_bounded(
    ...
    let mut prefix = [0_u8; SAFETENSORS_PREFIX_BYTES];
    file.read_exact(&mut prefix)?;
    let header_len = parse_header_length(prefix)?;
    if header_len > max_header_bytes {
        return Err(PlePackIoError::Format(
            "safetensors header exceeds configured read bound",
        ));
    }
    let data_section_start = data_section_start(header_len)?;
    if data_section_start > file_len {
        return Err(PlePackIoError::Format(
            "safetensors header extends beyond file",
        ));
    }
    let header_len = usize::try_from(header_len)
        .map_err(|_| PlePackIoError::Format("safetensors header length does not fit usize"))?;
    let mut header = vec![0_u8; header_len];
    file.read_exact(&mut header)?;
    parse_header_json(&header, data_section_start, file_len)
}

fn parse_header_length(prefix: [u8; SAFETENSORS_PREFIX_BYTES]) -> Result<u64, PlePackIoError> {
...
fn data_section_start(header_len: u64) -> Result<u64, PlePackIoError> {
    (SAFETENSORS_PREFIX_BYTES as u64)
...
fn parse_header_json(
    ...
    let header =
        parse_json_without_duplicate_members(header_bytes, "invalid safetensors header JSON")?;
    ...
    Ok(ParsedHeader {
        data_section_start,
        header_bytes_read: data_section_start,
        tensors,
    })
}
...
) -> Result<Value, PlePackIoError> {
    let mut deserializer = serde_json::Deserializer::from_slice(bytes);
    UniqueJsonMembers
        .deserialize(&mut deserializer)
        .and_then(|()| deserializer.end())
        .map_err(|_| PlePackIoError::Format(invalid_message))?;
    serde_json::from_slice(bytes).map_err(|_| PlePackIoError::Format(invalid_message))
}

struct UniqueJsonMembers;

impl<'de> DeserializeSeed<'de> for UniqueJsonMembers {
    type Value = ();
...
impl<'de> Visitor<'de> for UniqueJsonVisitor {
    ...
    where
        A: SeqAccess<'de>,
    {
        while sequence.next_element_seed(UniqueJsonMembers)?.is_some() {}
        Ok(())
    }
        ...
        A: MapAccess<'de>,
    {
        ...
        while let Some(name) = object.next_key::<String>()? {
            ...
            object.next_value_seed(UniqueJsonMembers)?;
        }
        Ok(())
    }
}
