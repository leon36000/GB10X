use gb10x_ple::{
    ExactPleRowSource, PlePackIoError, PlePackReader, PlePackWriter, SafetensorsPleManifest,
    SafetensorsPlePart, SafetensorsPleSource, plan_exact_layout,
};
...
fn write_raw_safetensors_header(path: &Path, header: &str, data: &[u8]) {
...
#[test]
fn duplicate_safetensors_header_members_are_rejected() {
    let dir = tempdir().unwrap();
    let data = vec![0_u8; ROW_BYTES * 2];
    let tensor = format!(
        r#"{{"dtype":"BF16","shape":[2,{ROW_ELEMENTS}],"data_offsets":[0,{}]}}"#,
        data.len()
    );
    let header = format!(r#"{{"ple.part0":{tensor},"ple.part0":{tensor}}}"#);
    write_raw_safetensors_header(&dir.path().join("part-000.safetensors"), &header, &data);
    let mut manifest = manifest();
    manifest.parts.truncate(1);
...
#[test]
fn safetensors_source_roundtrips_through_exact_overlay_and_binds_provenance() {
    let dir = tempdir().unwrap();
    let (r0, r1, r2, r3) = write_valid_fixture(dir.path());
    let manifest = manifest();
...

    {
        let source = SafetensorsPleSource::open(dir.path(), &manifest).expect("valid source");
        let plan =
            plan_exact_layout(4, ROW_BYTES as u32, 640, &[vec![1, 2]]).expect("valid layout");
        PlePackWriter::write_overlay(&pack, &source, &plan).expect("write exact overlay");

        let reader = PlePackReader::open(&pack, source).expect("open exact overlay");
        let mut actual = vec![0_u8; ROW_BYTES];
        for (logical_row, expected) in [(0, &r0), (1, &r1), (2, &r2), (3, &r3)] {
            reader
                .read_exact_row(logical_row, &mut actual)
                .expect("read exact row");
            assert_eq!(&actual, expected);
        }
        assert!(!reader.has_hot_overlay(0));
        assert!(reader.has_hot_overlay(1));
        assert!(reader.has_hot_overlay(2));
    ...
    fs::write(&path, bytes).unwrap();
    let changed_source =
        SafetensorsPleSource::open(dir.path(), &manifest).expect("reopen changed source");
    assert!(matches!(
        PlePackReader::open(&pack, changed_source),
